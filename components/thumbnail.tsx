...
export const Thumbnail = ({ src, fallback, isLive, username }: Props) => {
    ...
    if (!src) {
        content = (
            <div className="flex flex-col bg-background items-center justify-center gap-y-4 w-full h-full transition-transform group-hover:translate-x-2 group-hover:-translate-y-2 rounded-md">
                <UserAvatar
                    size="lg"
                    showBadge
                    username={username}
    ...
    return (
        <div className='group aspect-video rounded-md cursor-pointer relative'>
